...
class ESIC:
    ...
    def logStatus(self, level, message, screenshot=None):
        ...
        if self.dbObj is not None:
            ...
            self.dbObj.insertLog(self.refid, '{:%Y-%m-%d %H:%M:%S}'.format(nine_hours_from_now), level, message,
                                 'ESIC_registration', self.env, screenshot)
    ...
    def breakcaptcha(self):
        ...
        try:
            ...
            self.uploadToS3(os.path.join(screenshotName), 'ESIC/' + sname)
        except:
            pass
    ...
    def generate_ESIC(self,insuranceNumber):
        ...
        if r == 1:
            ...
            try:
                self.breakcaptcha()
                errorcode1 = self.driver.find_element_by_xpath("""// *[ @ id = "lblChallanMessage"]""")
                errorcode1 = errorcode1.text
                if errorcode1 == 'Invalid Captcha':
                    self.driver.find_element_by_xpath("""//*[@id="txtCaptcha"]""").clear()
                    self.breakcaptcha()
                    errorcode2 = self.driver.find_element_by_xpath("""// *[ @ id = "lblChallanMessage"]""")
                    errorcode2 = errorcode2.text
                    if errorcode2 == 'Invalid Captcha':
                        self.driver.find_element_by_xpath("""//*[@id="txtCaptcha"]""").clear()
                        self.breakcaptcha()
                        errorcode3 = self.driver.find_element_by_xpath("""// *[ @ id = "lblChallanMessage"]""")
                        errorcode3 = errorcode3.text
                        if errorcode3 == 'Invalid Captcha':
                            r = 2
                        pass
                ...
                pass
            try:
                errorcode33 = self.driver.find_element_by_xpath("""// *[ @ id = "lblMessage"] / b / font""")
                errorcode33 = errorcode33.text
                if errorcode33 == "IP Number 2612316927 is a duplicate of 2609450335 and duplicate number has been deactivated. Please make use of IP Number 2609450335":
                    dic = {}
            ...
            try:
                ...
                nomineePincode = self.driver.find_element_by_xpath("""//*[@id="ctl00_HomePageContent_lblPinCode"]""")
                nomineePincode = nomineePincode.text

                nomineeAddress = nomineeAddress1 +" "+ nomineedistrict +" "+nomineestate + " " + nomineePincode
...
if __name__ == '__main__':
    ...
    v = ESIC(refid="testing2", env = 'prod')
    data = v.ESIF_response(insuranceNumber = "2612316941")
    print(data)